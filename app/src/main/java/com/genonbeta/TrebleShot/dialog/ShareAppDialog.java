    ...
    private void shareAsLink(@NonNull final Context context)
    {
        new Handler(Looper.myLooper()).post(new Runnable()
        {
            ...
            public void run()
            {
                try {
                    String textToShare;
                    textToShare = context.getString(R.string.text_linkTrebleshot,
                            AppConfig.URI_GOOGLE_PLAY);

                    //to-do Call from Backend API
                    // Or Persist in Config because it can Verify from Backend also
                    String fetchReferralCode = "XEROX50";
                    String referral_code = context.getString(R.string.text_link_referral_code, AppConfig.FIRST_TIME_REFFERAL);
                    textToShare = textToShare.concat(referral_code);
                    Intent sendIntent = new Intent(Intent.ACTION_SEND)
